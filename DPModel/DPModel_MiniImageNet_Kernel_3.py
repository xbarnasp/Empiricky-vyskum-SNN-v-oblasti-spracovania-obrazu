...
def main(lr=0.01, set_model=1):
    ...
    run_dir = f"./runs_DPModel_ker3_miniImageNet/run{file_id}"
    ...
    for batch in range(2):
        ...
        for i in range(len(train_data_paths)):
            if ((batch * 50) % 550) <= (i % 550) < (((batch + 1) * 50) % 550):
        ...
        if set_model == 1:
            model = DPModel3().to(device)
        elif set_model == 2:
            model = DPModel6().to(device)
        else:
            model = DPModel9().to(device)
...
if __name__ == "__main__":
    main(lr=0.001, set_model=1)
    # main(lr=0.001, set_model=2)
    # main(lr=0.001, set_model=3)
