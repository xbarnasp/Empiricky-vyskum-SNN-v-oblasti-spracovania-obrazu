...
DATA_DIR = Path("MiniImageNet_resized")
...
class VGG16_BachNorm(nn.Module):
...
def check_dataset_mean(data_loader):
    mean_sum = torch.zeros(3).to(device)
    total_pixels = 0
    ...
    print(f"Mean of the dataset after normalization: {dataset_mean.cpu().numpy()}")
...
def main(lr=0.01, set_model=1, batch=256):
    ...
    run_dir = f"./FinalTests/runs_VGG16_Mini-ImageNet/run{file_id}"
    ...
    for batch in range(1):
        ...
        data_path_validation = list(data_path_val.glob("*/*.JPEG"))
        ...
        if set_model == 1:
            ...
            model = VGG16_BachNorm().to(device)
        elif set_model == 3:
            model = VGG16_SNN().to(device)
        else:
            model = VGG16_SNN_AD().to(device)
...
if __name__ == "__main__":
    ...
    main(lr=0.001, set_model=2)  #BN_model
