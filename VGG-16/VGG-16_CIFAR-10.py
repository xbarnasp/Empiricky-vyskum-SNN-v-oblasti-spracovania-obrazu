...
def check_dataset_mean(data_loader):
    ...
    with torch.no_grad():
        for images, _ in data_loader:
            ...
            batch_pixels = images.shape[0] * images.shape[2] * images.shape[3]  # B * H * W
...
def main(lr=0.01, set_model=1, batch=256):
    ...
    run_dir = f"./FinalTests/runs_VGG16_cifar-10/run{file_id}"
    ...
    for batch in range(3):
        ...
        for i in range(len(train_data_paths)):
            if ((batch * 500) % 5000) <= (i % 5000) < (((batch + 1) * 500) % 5000):
